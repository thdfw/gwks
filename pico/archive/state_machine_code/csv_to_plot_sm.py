...
def animate(i):
    ...
    # Split timesteps into sections

    # Plot results
    plt.plot(list(df.timestamp), list(df.freq), alpha=0.75, label=f'Hall effect', color='red')
    #plt.scatter(list(df.timestamp), list(df.hall_GPM), alpha=0.15, color='black')
